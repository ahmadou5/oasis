...
import { formatAddress, formatFullTimestamp, formatRelativeTime } from "@/utils/tokenFormatters";
...
export function EnhancedTransactionView({ data }: EnhancedTransactionViewProps) {
  ...
  return (
    <div className="space-y-6">
        ...
        {data.swapInfo && (
          <div className="mb-6">
            <div className="flex items-center justify-center gap-6 mb-4">
              ...
              <div className="flex-1 max-w-xs">
                <div className="bg-red-900/20 border-2 border-red-800/50 rounded-xl p-4">
                  <div className="text-gray-400 text-xs uppercase mb-2">From</div>
            ...
            <div className="text-center text-gray-400 text-sm">
              ...
              {" "}that involves {data.swapInfo.fromToken.symbol}, {data.swapInfo.toToken.symbol}
            </div>
          </div>
        )}
      </div>
      ...
      <div className="bg-[#1a1a1a] border border-gray-800 rounded-xl overflow-hidden">
        ...
        {/* Tab Content */}
        <div className="p-6">
          {activeTab === "summary" && <SummaryTab data={data} CopyButton={CopyButton} />}
          {activeTab === "balances" && <BalancesTab data={data} CopyButton={CopyButton} />}
          {activeTab === "instructions" && <InstructionsTab data={data} CopyButton={CopyButton} />}
          {activeTab === "logs" && <LogsTab data={data} />}
          {activeTab === "raw" && <RawTab data={data} CopyButton={CopyButton} />}
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-[#1a1a1a] border border-gray-800 rounded-xl max-w-2xl w-full p-6">
            ...
            <div className="text-gray-300 space-y-3">
              <p>
                This transaction is a <strong className="text-white">{data.type}</strong> operation
                that {data.status === "success" ? "completed successfully" : "failed"}.
              </p>
              {data.swapInfo && (
                <p>
                  The user swapped <strong className="text-red-400">{data.swapInfo.fromToken.amount.toFixed(6)} {data.swapInfo.fromToken.symbol}</strong>
                  {" "}for <strong className="text-green-400">{data.swapInfo.toToken.amount.toFixed(6)} {data.swapInfo.toToken.symbol}</strong>
                  {" "}using {data.swapInfo.programName}.
                </p>
              )}
              <p>
                The transaction involved {data.programs.length} program{data.programs.length !== 1 ? "s" : ""}
                {" "}and consumed {data.computeUnitsConsumed.toLocaleString()} compute units.
              </p>
              <p>
                Total fee paid: <strong className="text-white">{(data.fee / 1e9).toFixed(9)} SOL</strong>
              </p>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <div className="flex items-center gap-2">
            <span className="text-white font-mono text-sm">{formatAddress(data.signature, 12, 12)}</span>
        ...
        <div>
          ...
          <div className="text-white text-sm">
            {formatRelativeTime(data.blockTime)} ({formatFullTimestamp(data.blockTime)})
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {data.programs.length > 3 && (
              <span className="text-gray-500 text-xs">+{data.programs.length - 3} more</span>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            ...
            className={`px-2 py-1 rounded text-xs ${
              showTokenAccounts ? "bg-red-900/30 text-red-400" : "bg-gray-800 text-gray-400"
            }`}
          >
            Show Token Accounts
          </button>
            ...
            {data.instructions.map((ix, index) => (
              <div key={index} className="bg-[#0d0d0d] border border-gray-800 rounded-lg p-4">
                <div className="text-gray-300 text-sm">
                  ...
                  {" "}Interact with{" "}
                  <span className="text-white font-medium">{ix.instructionType}</span>
                  {" "}through{" "}
                  <a
                ...
                {ix.accounts.length > 0 && (
                  <div className="mt-2 text-xs text-gray-500 space-y-1">
                    {ix.accounts.slice(0, 2).map((acc, i) => (
                      <div key={i}>
                        Account: <span className="font-mono">{formatAddress(acc.pubkey, 6, 6)}</span>
                        {acc.isSigner && <span className="ml-2 text-yellow-500">(Signer)</span>}
                        {acc.isWritable && <span className="ml-2 text-blue-500">(Writable)</span>}
                      </div>
                    ))}
                    {ix.accounts.length > 2 && (
                      <div className="text-gray-600">+ {ix.accounts.length - 2} more accounts</div>
                    )}
                  </div>
                )}
              </div>
      ...
      <div className="border-t border-gray-800 pt-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="flex justify-between">
            ...
            <div className="flex items-center gap-2">
              <span className="text-white font-mono text-sm">{formatAddress(data.feePayer, 6, 6)}</span>
          ...
          <div className="flex justify-between">
            ...
            <span className="text-white font-mono text-sm">{data.slot.toLocaleString()}</span>
          </div>

          <div className="flex justify-between">
            <span className="text-gray-500 text-sm">Compute Units Consumed</span>
            <span className="text-white font-mono text-sm">{data.computeUnitsConsumed.toLocaleString()}</span>
          </div>

          <div className="flex justify-between col-span-full">
            <span className="text-gray-500 text-sm">Recent Blockhash</span>
            <div className="flex items-center gap-2">
              <span className="text-white font-mono text-sm">{formatAddress(data.recentBlockhash, 8, 8)}</span>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            ...
            className={`px-3 py-1 rounded text-xs ${
              hideUnchanged ? "bg-red-900/30 text-red-400" : "bg-gray-800 text-gray-400"
            }`}
          >
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-800">
                <th className="text-left pb-3 text-gray-500 text-xs uppercase font-medium">#</th>
                <th className="text-left pb-3 text-gray-500 text-xs uppercase font-medium">Address</th>
                <th className="text-left pb-3 text-gray-500 text-xs uppercase font-medium">Owner</th>
                <th className="text-right pb-3 text-gray-500 text-xs uppercase font-medium">Post Balance</th>
                <th className="text-left pb-3 text-gray-500 text-xs uppercase font-medium">Details</th>
              </tr>
            </thead>
            <tbody>
              {filteredBalances.map((balance, index) => (
                <tr key={index} className="border-b border-gray-800/50 hover:bg-[#0d0d0d] transition-colors">
                  ...
                  <td className="py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <CopyButton text={balance.address} copyKey={`balance-${index}`} />
                    </div>
                  </td>
                  <td className="py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-400 text-sm">{balance.owner}</span>
                    </div>
                  </td>
                  <td className="py-3 text-right">
                    <div className="font-mono">
                      <div className={`text-sm ${balance.change > 0 ? "text-green-400" : balance.change < 0 ? "text-red-400" : "text-white"}`}>
                        {(balance.postBalance / 1e9).toFixed(9)} SOL
                      </div>
                      {balance.change !== 0 && (
                        <div className={`text-xs ${balance.change > 0 ? "text-green-400" : "text-red-400"}`}>
                          {balance.change > 0 ? "+" : ""}{(balance.change / 1e9).toFixed(9)}
                        </div>
                      )}
                    </div>
                  </td>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-800">
                  <th className="text-left pb-3 text-gray-500 text-xs uppercase font-medium">Owner</th>
                  <th className="text-left pb-3 text-gray-500 text-xs uppercase font-medium">Token Account</th>
                  <th className="text-left pb-3 text-gray-500 text-xs uppercase font-medium">Token Mint</th>
                  <th className="text-right pb-3 text-gray-500 text-xs uppercase font-medium">Change</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={index} className="border-b border-gray-800/50 hover:bg-[#0d0d0d] transition-colors">
                    ...
                    <td className="py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <CopyButton text={token.tokenAccount} copyKey={`token-${index}`} />
                      </div>
                    </td>
                    <td className="py-3">
                      <a
                    ...
                    <td className="py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <span className={`font-mono text-sm ${token.change > 0 ? "text-green-400" : "text-red-400"}`}>
                          {token.change > 0 ? "+" : ""}{token.change.toFixed(6)} {token.symbol}
                        </span>
                      </div>
                    </td>
                  </tr>
...
function InstructionsTab({
      ...
      return (
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-gray-500 text-xs">Length: {bytes.length} bytes</span>
            <button
            ...
            <div className="space-y-1">
              {hex.split('\n').map((line, i) => (
                <div key={i} className="flex gap-4">
                  <span className="text-gray-600 text-xs w-12">{(i * 16).toString(16).padStart(4, '0')}</span>
                  <span className="text-gray-300 text-xs font-mono">{line}</span>
                </div>
              ))}
            </div>
          )}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-white font-semibold text-lg">Transaction Instructions</h3>
          <p className="text-gray-500 text-sm mt-1">
            {data.instructions.length} instruction{data.instructions.length !== 1 ? 's' : ''} executed
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <button
            onClick={() => {
              const allIndices = new Set(data.instructions.map((_, i) => i));
              setExpandedInstructions(expandedInstructions.size === data.instructions.length ? new Set() : allIndices);
            }}
            className="px-3 py-1 rounded text-xs bg-gray-800 text-gray-400 hover:bg-gray-700"
          >
            {expandedInstructions.size === data.instructions.length ? "Collapse All" : "Expand All"}
          </button>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {data.instructions.map((instruction, index) => {
          ...
          const innerInstructionCount = instruction.innerInstructions?.length || 0;
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <div className="flex items-center gap-2">
                      <span className="text-white font-semibold">{instruction.programName}</span>
              ...
              {isExpanded && (
                <div className="border-t border-gray-800 p-4 space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <div className="text-gray-500 text-xs uppercase mb-2">Program</div>
                      <div className="flex items-center gap-2">
                        <div className="text-white text-sm">{instruction.programName}</div>
                        <CopyButton text={instruction.programId} copyKey={`prog-${index}`} />
                      </div>
                      <div className="text-gray-600 text-xs font-mono mt-1">{instruction.programId}</div>
                    </div>
                    <div>
                      <div className="text-gray-500 text-xs uppercase mb-2">Instruction Type</div>
                      <div className="text-white text-sm font-mono">{instruction.instructionType}</div>
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="flex gap-2 text-xs">
                        <span className="px-2 py-0.5 bg-yellow-900/20 text-yellow-400 rounded">Signer</span>
                        <span className="px-2 py-0.5 bg-blue-900/20 text-blue-400 rounded">Writable</span>
                      </div>
                    </div>
                    <div className="overflow-x-auto">
                      <table className="w-full">
                        <thead>
                          <tr className="border-b border-gray-800">
                            <th className="text-left pb-2 text-gray-600 text-xs w-12">#</th>
                            <th className="text-left pb-2 text-gray-600 text-xs">Account Address</th>
                            <th className="text-left pb-2 text-gray-600 text-xs">Permissions</th>
                            <th className="text-left pb-2 text-gray-600 text-xs">Actions</th>
                          </tr>
                        </thead>
                        <tbody>
                          {instruction.accounts.map((account, accIndex) => (
                            <tr key={accIndex} className="border-b border-gray-800/50 hover:bg-black/20">
                              <td className="py-3 text-gray-500 text-xs font-mono">{accIndex}</td>
                              <td className="py-3">
                                <a
                              ...
                              <td className="py-3">
                                <CopyButton text={account.pubkey} copyKey={`acc-${index}-${accIndex}`} />
                              </td>
                            </tr>
                          ))}
                        </tbody>
                  ...
                  {/* Instruction Data */}
                  <div>
                    <div className="text-gray-500 text-xs uppercase mb-3">Instruction Data</div>
                    ...
                    <div>
                      ...
                      <div className="space-y-3 pl-4 border-l-2 border-blue-800">
                        {instruction.innerInstructions.map((inner, innerIndex) => (
                          <div key={innerIndex} className="bg-black/30 border border-gray-800 rounded-lg p-4">
                            <div className="flex items-center justify-between mb-3">
                              <div>
                                <div className="flex items-center gap-2">
                                  <span className="text-blue-400 text-xs font-mono">#{innerIndex}</span>
                                  <span className="text-white font-medium">{inner.programName}</span>
                                </div>
                                <div className="text-gray-500 text-xs font-mono mt-1">
                                  {formatAddress(inner.programId, 8, 8)}
                                </div>
                              </div>
                              <CopyButton text={inner.programId} copyKey={`inner-${index}-${innerIndex}`} />
                            </div>
                            
                            {inner.accounts && inner.accounts.length > 0 && (
                              <div className="mt-3">
                                <div className="text-gray-600 text-xs mb-2">{inner.accounts.length} accounts</div>
                                <div className="space-y-1">
                                  {inner.accounts.slice(0, 3).map((acc, i) => (
                                    <div key={i} className="text-gray-500 text-xs font-mono flex items-center gap-2">
                                      <span className="text-gray-700">#{i}</span>
                                      <a href={`/account/${acc.pubkey}`} className="text-emerald-400 hover:text-emerald-300">
                                        {formatAddress(acc.pubkey, 6, 6)}
                                      </a>
                                      {acc.isSigner && <span className="text-yellow-600 text-xs">S</span>}
                                      {acc.isWritable && <span className="text-blue-600 text-xs">W</span>}
                                    </div>
                                  ))}
                                  {inner.accounts.length > 3 && (
                                    <div className="text-gray-700 text-xs">+ {inner.accounts.length - 3} more</div>
                                  )}
                                </div>
                              </div>
                            )}
...
function LogsTab({ data }: { data: TransactionDetail }) {
  ...
  const groupedLogs: Array<{ program: string; logs: string[]; depth: number; success: boolean; failed: boolean }> = [];
  let currentGroup: { program: string; logs: string[]; depth: number; success: boolean; failed: boolean } | null = null;
  let depthStack = 0;
  ...
  data.logs.forEach((log, index) => {
    ...
    if (invokeMatch) {
      ...
      if (consumedMatch || successMatch || failedMatch) {
        const logProgram = consumedMatch?.[1] || successMatch?.[1] || failedMatch?.[1];
  ...
  const filteredGroups = searchTerm
    ? groupedLogs.filter(group =>
        group.logs.some(log => log.toLowerCase().includes(searchTerm.toLowerCase())) ||
        (KNOWN_PROGRAMS[group.program] || "Unknown").toLowerCase().includes(searchTerm.toLowerCase())
      )
    : groupedLogs;
  ...
  const getLogStyle = (log: string): string => {
    ...
    if (log.includes("failed") || log.includes("error")) return "text-red-400 font-semibold";
    ...
    if (searchTerm && log.toLowerCase().includes(searchTerm.toLowerCase())) {
      ...
      return (
        <div className="flex gap-3">
          {showLineNumbers && (
            <span className="text-gray-700 text-xs w-8 text-right flex-shrink-0">{lineNumber}</span>
          )}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-white font-semibold text-lg">Program Execution Logs</h3>
          <p className="text-gray-500 text-sm mt-1">
            {filteredGroups.length} program{filteredGroups.length !== 1 ? 's' : ''} executed • {data.logs.length} log entries
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-3 py-1 rounded text-xs ${
              showLineNumbers ? "bg-red-900/30 text-red-400" : "bg-gray-800 text-gray-400"
            }`}
          >
          ...
          <button
            onClick={() => {
              ...
              setExpandedSections(expandedSections.size === filteredGroups.length ? new Set() : allIndices);
            }}
            className="px-3 py-1 rounded text-xs bg-gray-800 text-gray-400 hover:bg-gray-700"
          >
            {expandedSections.size === filteredGroups.length ? "Collapse All" : "Expand All"}
          </button>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {filteredGroups.map((group, index) => {
          ...
          const programName = KNOWN_PROGRAMS[group.program] || "Unknown Program";
          const computedUnits = group.logs.find(log => log.includes("consumed"))?.match(/(\d+)/)?.[0];
            ...
            >
              ...
              >
                <div className="flex-1">
                  <div className="flex items-center gap-3">
                    <div className={`w-2 h-2 rounded-full ${
                      group.failed ? "bg-red-500" : group.success ? "bg-green-500" : "bg-gray-500"
                    }`} />
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-gray-600 text-xs">{group.logs.length} logs</span>
                  {isExpanded ? (
                    <ChevronDown className="w-5 h-5 text-gray-400" />
                  ) : (
                    <ChevronRight className="w-5 h-5 text-gray-400" />
      ...
      {filteredGroups.length === 0 && (
        <div className="text-center py-12 bg-[#0d0d0d] border border-gray-800 rounded-lg">
          ...
          <div className="text-gray-400">No logs found matching "{searchTerm}"</div>
        </div>
      )}
    </div>
  );
...
function RawTab({
  ...
  const JSONTreeNode = ({ value, path = 'root', depth = 0 }: { value: any; path?: string; depth?: number }) => {
    ...
    if (isPrimitive) {
      return (
        <span className={`${
          ...
          value === null ? 'text-red-400' : 'text-gray-300'
        }`}>
        ...
        {isExpanded && (
          <div className="ml-4 border-l border-gray-800 pl-3 mt-1">
              ...
              return (
                <div key={key} className="py-0.5">
                  <div className="flex items-start gap-2">
                    ...
                    <JSONTreeNode value={childValue} path={childPath} depth={depth + 1} />
                  </div>
                </div>
              );
            })}
            <div className="text-gray-400 -ml-4">
              {isArray ? ']' : '}'}
              {depth === 0 && <span className="text-gray-600 text-xs ml-2">// {itemCount} total</span>}
            </div>
          </div>
        )}
      </div>
  ...
  const syntaxHighlightJSON = (json: string): JSX.Element[] => {
    ...
    return lines.map((line, index) => {
      ...
      // Property names
      highlightedLine = highlightedLine.replace(/"([^"]+)":/g, '<span class="text-blue-300">"$1":</span>');
      
      // String values
      highlightedLine = highlightedLine.replace(/: "([^"]*)"/g, ': <span class="text-green-400">"$1"</span>');
      
      // Numbers
      highlightedLine = highlightedLine.replace(/: (-?\d+\.?\d*)/g, ': <span class="text-blue-400">$1</span>');
      
      // Booleans
      highlightedLine = highlightedLine.replace(/: (true|false)/g, ': <span class="text-purple-400">$1</span>');
      
      // Null
      highlightedLine = highlightedLine.replace(/: (null)/g, ': <span class="text-red-400">$1</span>');
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-white font-semibold text-lg">Raw Transaction Data</h3>
          <p className="text-gray-500 text-sm mt-1">
      ...
      <div className="bg-[#0d0d0d] border border-gray-800 rounded-lg overflow-hidden">
        <div className="border-b border-gray-800 p-3 flex items-center justify-between bg-black/30">
          ...
          <div className="text-gray-600 text-xs">
            {viewMode === 'tree' ? 'Click to expand/collapse' : 'Syntax highlighted'}
          </div>
        </div>
      ...
      <div className="bg-[#0d0d0d] border border-gray-800 rounded-lg p-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            ...
            <div className="text-gray-500 text-xs">Fee, status, compute units</div>
          </button>
          <button
            onClick={() => {
              setViewMode('tree');
          ...
          >
            ...
            <div className="text-gray-500 text-xs">Instructions, accounts, signatures</div>
          </button>
        </div>
      </div>
    </div>
