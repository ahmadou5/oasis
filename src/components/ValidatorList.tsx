...
export function ValidatorList({ showFilters = true }: ValidatorListProps) {
  ...
  const [selectedValidatorForStaking, setSelectedValidatorForStaking] = useState<ValidatorInfo | null>(null)
  const [currentPage, setCurrentPage] = useState(1)
  const [pageSize, setPageSize] = useState(20)
  const [searchTerm, setSearchTerm] = useState('')
  const [sortBy, setSortBy] = useState<'apy' | 'stake' | 'commission' | 'name'>('apy')
    ...
    return (
      <div className="card text-center py-12">
        <div className="text-red-400 mb-4">
          ...
          <h3 className="text-xl font-semibold mb-2">Failed to Load Validators</h3>
    ...
    <div className="space-y-6">
      ...
      {validators.length > 0 && stats.totalValidators > 0 && (
        <div className="card mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <div className="text-2xl font-bold text-solana-green">{stats.activeValidators}</div>
              <div className="text-sm text-gray-500 dark:text-solana-gray-400">Active Validators</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-solana-blue">{(stats.totalStake / 1_000_000).toFixed(1)}M</div>
              <div className="text-sm text-gray-500 dark:text-solana-gray-400">Total Stake (SOL)</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-solana-purple">{stats.averageApy.toFixed(2)}%</div>
              <div className="text-sm text-gray-500 dark:text-solana-gray-400">Avg APY</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-yellow-400">{stats.averageCommission.toFixed(1)}%</div>
      ...
      {paginatedValidators.length === 0 ? (
        <div className="card text-center py-12">
          <div className="text-solana-gray-400">
            <svg className="w-16 h-16 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2M4 13h2m13-8V4a1 1 0 00-1-1H7a1 1 0 00-1 1v1m7-1v1" />
            </svg>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center items-center gap-4 mt-8">
          ...
          <span className="text-gray-600 dark:text-solana-gray-400">
            Page {currentPage} of {totalPages}
          </span>
