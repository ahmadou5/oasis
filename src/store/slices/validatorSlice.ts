...
const validatorSlice = createSlice({
  ...
  initialState,
  reducers: {
    setSelectedValidator: (state, action: PayloadAction<ValidatorInfo | null>) => {
      state.selectedValidator = action.payload
    },
    updateFilters: (state, action: PayloadAction<Partial<ValidatorFilters>>) => {
...
function applyFilters(validators: ValidatorInfo[], filters: ValidatorFilters): ValidatorInfo[] {
...
function applyPagination(state: ValidatorState) {
  ...
  state.displayedValidators = state.filteredValidators.slice(startIndex, endIndex)
}

export const { setSelectedValidator, updateFilters, updatePagination, clearError } = validatorSlice.actions
