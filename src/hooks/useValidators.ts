...
export function useValidators() {
  ...
  const fetchSolanaBeachMetadata = useCallback(async (): Promise<Map<string, SolanaBeachValidator>> => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Solana Beach API error: ${response.status}`)
      }
  ...
  const fetchValidatorPerformance = useCallback(async (validatorAddress: string): Promise<{
    ...
    try {
      const response = await fetch(
        buildSolanaBeachURL(ENV.SOLANA_BEACH.ENDPOINTS.VALIDATOR_EPOCHS(validatorAddress, 30)),
        {
          headers: getSolanaBeachHeaders()
        }
      )
      ...
      const epochData: SolanaBeachValidatorPerformance[] = await response.json()
      ...
      const averageApy = validApyData.length > 0 
        ? validApyData.reduce((sum, e) => sum + (e.apy || 0), 0) / validApyData.length 
        : 0
      
      // Calculate uptime
      const totalEpochs = epochData.length
      const activeEpochs = epochData.filter(e => !e.delinquent && (e.credits || 0) > 0).length
      const uptime = totalEpochs > 0 ? (activeEpochs / totalEpochs) * 100 : 0
      
      return { performanceHistory, averageApy, uptime }
    } catch (error) {
      console.warn(`⚠️ Failed to fetch performance for ${validatorAddress}:`, error)
      return { performanceHistory: [], averageApy: 0, uptime: 0 }
    }
  }, [])

  // Calculate APY based on epoch credits (fallback when Solana Beach data unavailable)
  const calculateAPYFromCredits = useCallback((epochCredits: [number, number, number][]): number => {
  ...
  const calculateSkipRate = useCallback((epochCredits: [number, number, number][]): number => {
  ...
  const fetchValidators = useCallback(async () => {
    ...
    try {
      ...
      console.log(`📊 RPC Data: ${voteAccounts.current.length} active, ${voteAccounts.delinquent.length} delinquent validators`)
      ...
      for (const account of topValidators) {
        try {
          ...
          if (stakeAmount > 1_000_000) { // Only fetch for validators with >1M SOL stake
          ...
          const validatorInfo: ValidatorInfo = {
            ...
            name: beachData?.name || `Validator ${account.votePubkey.slice(0, 8)}...`,
            commission: account.commission,
            stake: stakeAmount,
            apy: performanceData.averageApy > 0 ? performanceData.averageApy : rpcApy,
            delegatedStake: stakeAmount,
            skipRate: rpcSkipRate,
            dataCenter: beachData?.location || beachData?.country || 'Unknown',
            website: beachData?.website,
            description: beachData?.description || beachData?.details,
            avatar: beachData?.avatar,
            status: account.status,
            epochCredits: account.epochCredits.map(([epoch, credits]) => credits).slice(-10),
            ...
            performanceHistory: performanceData.performanceHistory.map((p: SolanaBeachValidatorPerformance) => ({
              epoch: p.epoch,
              apy: p.apy || 0,
          ...
          console.warn(`⚠️ Failed to process validator ${account.votePubkey}:`, error)
      ...
      const activeValidators = processedValidators.filter(v => v.status === 'active')
      const totalStake = processedValidators.reduce((sum, v) => sum + v.stake, 0)
      const averageApy = activeValidators.length > 0 
        ? activeValidators.reduce((sum, v) => sum + v.apy, 0) / activeValidators.length 
        : 0
      const averageCommission = processedValidators.length > 0 
        ? processedValidators.reduce((sum, v) => sum + v.commission, 0) / processedValidators.length 
        : 0
      ...
      console.log(`✅ Successfully processed ${processedValidators.length} validators with enhanced data`)

    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Failed to fetch validator data'
      console.error('❌ Validator fetch error:', error)
      setError(errorMessage)
    } finally {
      setLoading(false)
    }
  }, [connection, fetchSolanaBeachMetadata, fetchValidatorPerformance, calculateAPYFromCredits, calculateSkipRate])
